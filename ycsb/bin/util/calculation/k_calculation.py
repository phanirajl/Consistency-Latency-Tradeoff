...
# Given a list of writes, construct all permutations.
# def all_permutations(lst, begin, end):
# 	global write_orders
# 	if begin >= end:
# 		write_orders.append(lst[:])
# 	else:
# 		current = begin
# 		for index in range(begin, end):
# 			lst[index], lst[current] = lst[current], lst[index]
# 			all_permutations(lst, begin+1, end)
# 			lst[index], lst[current] = lst[current], lst[index]
...
def k_calculation(chunk_dict, cluster_dict, max_chunk_size, write_start, write_finish, read_start, read_finish):
	...
	# e.g. read_count_list[0] = 100 : Meaning 100 reads return the newest value.
	# e.g. read_count_list[2] = 30 : Meaning 30 reads return the latest 3( not 2) value
	...
	for vers in chunk_dict.values():
		...
		for write_order in write_orders:
			...
			try:
				...
				# use k_count's opposite number for later's best-order search:
