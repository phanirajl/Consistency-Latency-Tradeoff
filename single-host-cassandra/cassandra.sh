...
for subdir in ${dirarr[@]}; do
    ...
    if [[ $match = "$subdir" ]]; then
        ...
        ./cassandra/bin/cassandra -p "$dir/$subdir/pid" -Dcassandra.logdir="$dir/$subdir/logs" -Dcassandra.storagedir="$dir/$subdir/data" -Dlogback.configurationFile="$dir/$subdir/conf/logback.xml" $short $long
        sleep 60
    fi
done
